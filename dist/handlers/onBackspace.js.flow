...
function onBackspace(
    ...
    let selection = value.selection.moveToStart();
    const ignoreFirstCell = selection.isCollapsed && 
        selection.start.isAtEndOfNode(cells.first());
    selection = value.selection.moveToEnd();
    const ignoreLastCell = selection.isCollapsed &&
        selection.start.isAtStartOfNode(cells.last());
