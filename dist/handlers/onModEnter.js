...
function onModEnter(event, change, editor, opts) {
    ...
    var table = _utils.TablePosition.create(opts, value.document, value.selection.start.key).table;
    var tableParent = value.document.getParent(table.key);
    var insertionIndex = tableParent.nodes.indexOf(table) + 1;

    return change.insertNodeByKey(tableParent.key, insertionIndex, exitBlock).moveToStartOfNode(exitBlock);
}
